...
public class StructurePoolBasedGenerator {
   ...
   public static void generate(DynamicRegistryManager dynamicRegistries, StructurePoolFeatureConfig config, StructurePoolBasedGenerator.PieceFactory pieceFactory, ChunkGenerator chunkGenerator, StructureManager structureManager, BlockPos pos, StructurePiecesHolder children, Random random, boolean modifyBoundingBox, boolean surface, HeightLimitView world) {
      ...
      if (structurePoolElement != EmptyPoolElement.INSTANCE) {
         ...
         int l;
         if (surface) {
            l = pos.getY() + chunkGenerator.getHeightOnGround(i, j, Heightmap.Type.WORLD_SURFACE_WG, world);
         } else {
            l = pos.getY();
         }
         ...
         if (config.getSize() > 0) {
            ...
            StructurePoolBasedGenerator.StructurePoolGenerator structurePoolGenerator = new StructurePoolBasedGenerator.StructurePoolGenerator(registry, config.getSize(), pieceFactory, chunkGenerator, structureManager, list, random);

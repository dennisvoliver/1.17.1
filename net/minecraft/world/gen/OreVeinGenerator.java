...
public class OreVeinGenerator {
   ...
   private static final double ORE_FREQUENCY_NOISE_SCALE = 1.5D;
   ...
   public OreVeinGenerator(long seed, BlockState defaultState, int horizontalNoiseResolution, int verticalNoiseResolution, int minY) {
      ...
      this.oreFrequencyNoiseSampler = DoublePerlinNoiseSampler.create(new SimpleRandom(random.nextLong()), -8, (double[])(1.0D));
      this.firstOrePlacementNoiseSampler = DoublePerlinNoiseSampler.create(new SimpleRandom(random.nextLong()), -7, (double[])(1.0D));
      this.secondOrePlacementNoiseSampler = DoublePerlinNoiseSampler.create(new SimpleRandom(random.nextLong()), -7, (double[])(1.0D));
      this.oreChanceNoiseSampler = DoublePerlinNoiseSampler.create(new SimpleRandom(0L), -5, (double[])(1.0D));
      this.horizontalNoiseResolution = horizontalNoiseResolution;
      this.verticalNoiseResolution = verticalNoiseResolution;
      this.maxY = Stream.of(OreVeinGenerator.VeinType.values()).mapToInt((veinType) -> {
         return veinType.maxY;
      }).max().orElse(minY);
      this.minY = Stream.of(OreVeinGenerator.VeinType.values()).mapToInt((veinType) -> {
         return veinType.minY;
      }).min().orElse(minY);
   }

   public void sampleOreFrequencyNoise(double[] buffer, int x, int z, int minY, int noiseSizeY) {
      this.sample(buffer, x, z, this.oreFrequencyNoiseSampler, 1.5D, minY, noiseSizeY);
   }

   public void sampleFirstOrePlacementNoise(double[] buffer, int x, int z, int minY, int noiseSizeY) {
      this.sample(buffer, x, z, this.firstOrePlacementNoiseSampler, 4.0D, minY, noiseSizeY);
   }

   public void sampleSecondOrePlacementNoise(double[] buffer, int x, int z, int minY, int noiseSizeY) {
      this.sample(buffer, x, z, this.secondOrePlacementNoiseSampler, 4.0D, minY, noiseSizeY);
   }
   ...
   public void sample(double[] buffer, int x, int z, DoublePerlinNoiseSampler sampler, double scale, int minY, int noiseSizeY) {
      for(int i = 0; i < noiseSizeY; ++i) {
         ...
         if (l >= this.minY && l <= this.maxY) {
            e = sampler.sample((double)k * scale, (double)l * scale, (double)m * scale);
         } else {
            e = 0.0D;
         }
   ...
   public BlockState sample(WorldGenRandom random, int x, int y, int z, double oreFrequencyNoise, double firstOrePlacementNoise, double secondOrePlacementNoise) {
      ...
      if (veinType == null) {
         ...
         double d = MathHelper.clampedLerpFromProgress(Math.abs(oreFrequencyNoise), 0.5D, 0.6000000238418579D, 0.10000000149011612D, 0.30000001192092896D);
         if ((double)random.nextFloat() < d && this.oreChanceNoiseSampler.sample((double)x, (double)y, (double)z) > -0.30000001192092896D) {
   ...
   private boolean shouldPlaceOreBlock(double firstOrePlacementNoise, double secondOrePlacementNoise) {
      double d = Math.abs(1.0D * firstOrePlacementNoise) - 0.07999999821186066D;
      double e = Math.abs(1.0D * secondOrePlacementNoise) - 0.07999999821186066D;
      return Math.max(d, e) < 0.0D;
   }

   @Nullable
   private OreVeinGenerator.VeinType getVeinType(double oreFrequencyNoise, int y) {
      OreVeinGenerator.VeinType veinType = oreFrequencyNoise > 0.0D ? OreVeinGenerator.VeinType.COPPER : OreVeinGenerator.VeinType.IRON;
      int i = veinType.maxY - y;
      int j = y - veinType.minY;
      if (j >= 0 && i >= 0) {
         int k = Math.min(i, j);
         double d = MathHelper.clampedLerpFromProgress((double)k, 0.0D, 20.0D, -0.2D, 0.0D);

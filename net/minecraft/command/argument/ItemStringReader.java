...
public class ItemStringReader {
   ...
   public static final DynamicCommandExceptionType ID_INVALID_EXCEPTION = new DynamicCommandExceptionType((id) -> {
      return new TranslatableText("argument.item.id.invalid", new Object[]{id});
   });
   private static final char LEFT_CURLY_BRACKET = '{';
   private static final char HASH_SIGN = '#';
   private static final BiFunction<SuggestionsBuilder, TagGroup<Item>, CompletableFuture<Suggestions>> NBT_SUGGESTION_PROVIDER = (builder, group) -> {
   ...
   private CompletableFuture<Suggestions> suggestTag(SuggestionsBuilder builder, TagGroup<Item> group) {
      return CommandSource.suggestIdentifiers((Iterable)group.getTagIds(), builder.createOffset(this.cursor));
   }
   ...
   private CompletableFuture<Suggestions> suggestAny(SuggestionsBuilder builder, TagGroup<Item> group) {
      ...
      return CommandSource.suggestIdentifiers((Iterable)Registry.ITEM.getIds(), builder);
   }

   public CompletableFuture<Suggestions> getSuggestions(SuggestionsBuilder builder, TagGroup<Item> group) {
      return (CompletableFuture)this.suggestions.apply(builder.createOffset(this.reader.getCursor()), group);
   }
}

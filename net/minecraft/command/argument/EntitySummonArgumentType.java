...
public class EntitySummonArgumentType implements ArgumentType<Identifier> {
   ...
   public static final DynamicCommandExceptionType NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType((id) -> {
      return new TranslatableText("entity.notFound", new Object[]{id});
   });
   ...
   private static Identifier validate(Identifier id) throws CommandSyntaxException {
      Registry.ENTITY_TYPE.getOrEmpty(id).filter(EntityType::isSummonable).orElseThrow(() -> {
         return NOT_FOUND_EXCEPTION.create(id);
      });
      return id;
   }

...
public class Format4ResourcePack implements ResourcePack {
   ...
   public static InputStream openCroppedStream(InputStream base, InputStream overlay, int width, int left, int top, int right, int bottom) throws IOException {
      ...
      label105: {
         try {
            ...
            label107: {
               nativeImage2 = NativeImage.read(overlay);
   ...
   static {
      SHIELD_PATTERN_TEXTURES = (Set)BANNER_PATTERN_TYPES.stream().map((patternName) -> {
         return "textures/entity/shield/" + patternName + ".png";
      }).collect(Collectors.toSet());
      BANNER_PATTERN_TEXTURES = (Set)BANNER_PATTERN_TYPES.stream().map((patternName) -> {
         return "textures/entity/banner/" + patternName + ".png";
      }).collect(Collectors.toSet());

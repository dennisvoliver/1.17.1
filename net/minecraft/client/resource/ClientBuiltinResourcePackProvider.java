   ...
   public CompletableFuture<?> download(String url, String packSha1, boolean closeAfterDownload) {
      ...
      try {
         ...
         if (file.exists()) {
            ...
            completableFuture2 = NetworkUtils.downloadResourcePack(file, url, map, 104857600, progressScreen, minecraftClient.getNetworkProxy());
         }
         ...
         this.downloadTask = completableFuture2.thenCompose((object) -> {
            ...
            if (throwable != null) {
               ...
               MinecraftClient minecraftClient = MinecraftClient.getInstance();
               minecraftClient.execute(() -> {
                  minecraftClient.setScreen(new ConfirmScreen((confirmed) -> {
                     if (confirmed) {

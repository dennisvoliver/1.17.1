...
public class RealmsSlotOptionsScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.difficulty = (Difficulty)get(DIFFICULTIES, this.options.difficulty, 2);
      this.gameMode = (GameMode)get(GAME_MODES, this.options.gameMode, 0);
      if (this.worldType == RealmsServer.WorldType.NORMAL) {
         ...
         this.addLabel(new RealmsLabel(text3, this.width / 2, 26, 16711680));
         this.pvp = true;
      ...
      this.addDrawableChild(CyclingButtonWidget.builder(GameMode::getSimpleTranslatableName).values(GAME_MODES).initially(this.gameMode).build(this.column1_x, row(3), this.column2_x, 20, new TranslatableText("selectWorld.gameMode"), (button, gameModeIndex) -> {
      ...
      CyclingButtonWidget<Boolean> cyclingButtonWidget3 = CyclingButtonWidget.onOffBuilder(this.difficulty != Difficulty.PEACEFUL && this.spawnMonsters).build(i, row(5), this.column2_x, 20, new TranslatableText("mco.configure.world.spawnMonsters"), (button, spawnMonsters) -> {
         this.spawnMonsters = spawnMonsters;
      });
      this.addDrawableChild(CyclingButtonWidget.builder(Difficulty::getTranslatableName).values(DIFFICULTIES).initially(this.difficulty).build(this.column1_x, row(5), this.column2_x, 20, new TranslatableText("options.difficulty"), (button, difficulty) -> {
      ...
      this.spawnProtectionButton = (RealmsSlotOptionsScreen.SettingsSlider)this.addDrawableChild(new RealmsSlotOptionsScreen.SettingsSlider(this.column1_x, row(7), this.column2_x, this.spawnProtection, 0.0F, 16.0F));
   ...
   private class SettingsSlider extends SliderWidget {
      ...
      protected void updateMessage() {
         this.setMessage(ScreenTexts.composeGenericOptionText(RealmsSlotOptionsScreen.SPAWN_PROTECTION, (Text)(RealmsSlotOptionsScreen.this.spawnProtection == 0 ? ScreenTexts.OFF : new LiteralText(String.valueOf(RealmsSlotOptionsScreen.this.spawnProtection)))));
      }

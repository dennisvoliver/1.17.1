...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
   ...
   public RealmsSelectWorldTemplateScreen(Text title, Consumer<WorldTemplate> callback, RealmsServer.WorldType worldType, @Nullable WorldTemplatePaginatedList templateList) {
      ...
      if (templateList == null) {
         ...
         this.templateList = new RealmsSelectWorldTemplateScreen.WorldTemplateObjectSelectionList(Lists.newArrayList((Iterable)templateList.templates));
   ...
   private void setPagination(final WorldTemplatePaginatedList templateList) {
      (new Thread("realms-template-fetcher") {
         public void run() {
            ...
            Either either;
            for(RealmsClient realmsClient = RealmsClient.createRealmsClient(); worldTemplatePaginatedList != null; worldTemplatePaginatedList = (WorldTemplatePaginatedList)RealmsSelectWorldTemplateScreen.this.client.submit(() -> {
   ...
   private void renderMessages(MatrixStack matrices, int x, int y, List<TextRenderingUtils.Line> messages) {
      for(int i = 0; i < messages.size(); ++i) {
         ...
         for(Iterator var10 = line.segments.iterator(); var10.hasNext(); l = n) {
            ...
            n = this.textRenderer.drawWithShadow(matrices, lineSegment.renderedText(), (float)l, (float)j, m);
   ...
   protected void renderMousehoverTooltip(MatrixStack matrices, @Nullable Text tooltip, int mouseX, int mouseY) {
      if (tooltip != null) {
         int i = mouseX + 12;
         int j = mouseY - 12;
         int k = this.textRenderer.getWidth((StringVisitable)tooltip);
   ...
   private class WorldTemplateObjectSelectionListEntry extends AlwaysSelectedEntryListWidget.Entry<RealmsSelectWorldTemplateScreen.WorldTemplateObjectSelectionListEntry> {
      ...
      private void renderWorldTemplateItem(MatrixStack matrices, WorldTemplate template, int x, int y, int mouseX, int mouseY) {
         ...
         RealmsSelectWorldTemplateScreen.this.textRenderer.draw(matrices, template.version, (float)(i + 227 - RealmsSelectWorldTemplateScreen.this.textRenderer.getWidth(template.version)), (float)(y + 1), 7105644);

...
public class RealmsMainScreen extends RealmsScreen {
         ...
         if (realmsDataFetcher.isFetchedSinceLastTry(RealmsDataFetcher.Task.SERVER_LIST)) {
            ...
            if (list != null) {
               ...
               while(var4.hasNext()) {
                  ...
                  if (this.isOwnedNotExpired(realmsServer3)) {
                     bl2 = true;
                  }
               }
            ...
            public void run() {
               ...
               try {
                  ...
                  if (var3.httpResultCode == 401) {
                     ...
                     RealmsMainScreen.this.client.execute(() -> {
                        RealmsMainScreen.this.client.setScreen(new RealmsGenericErrorScreen(var3, RealmsMainScreen.this.lastScreen));
                     });
                  }
               }
         ...
         public void run() {
            ...
            try {
               ...
               if (boolean_) {
                  ...
                  RealmsMainScreen.this.client.execute(() -> {
                     RealmsMainScreen.this.client.setScreen(new RealmsParentalConsentScreen(RealmsMainScreen.this.lastScreen));
                  });
               }
         ...
         (new Thread("Realms-leave-server") {
            public void run() {
               try {
                  ...
                  RealmsMainScreen.this.client.execute(() -> {
                     RealmsMainScreen.this.client.setScreen(new RealmsGenericErrorScreen(var3, RealmsMainScreen.this));
                  });
               }
   ...
   void removeServer(RealmsServer serverData) {
      ...
      this.realmSelectionList.children().removeIf((child) -> {
         return child instanceof RealmsMainScreen.RealmSelectionListEntry && ((RealmsMainScreen.RealmSelectionListEntry)child).mServerData.id == this.selectedServerId;
      });
   ...
   void drawInvitationPendingIcon(MatrixStack matrices, int mouseX, int mouseY, int x, int y, boolean hovered, boolean active) {
      ...
      if (bl4) {
         ...
         DrawableHelper.drawTexture(matrices, x + 4, y + 4 + l, (float)m, h, 8, 8, 48, 16);
      }
      ...
      if (bl5) {
         ...
         int o = this.textRenderer.getWidth((StringVisitable)text);
         this.fillGradient(matrices, m - 3, mouseY - 3, m + o + 3, mouseY + 8 + 3, -1073741824, -1073741824);
         this.textRenderer.drawWithShadow(matrices, (Text)text, (float)m, (float)mouseY, -1);
      }
   ...
   void drawExpiring(MatrixStack matrices, int x, int y, int mouseX, int mouseY, int remainingDays) {
      ...
      if (mouseX >= x && mouseX <= x + 9 && mouseY >= y && mouseY <= y + 27 && mouseY < this.height - 40 && mouseY > 32 && !this.shouldShowPopup()) {
         if (remainingDays <= 0) {
            ...
            this.setTooltips(new TranslatableText("mco.selectServer.expires.days", new Object[]{remainingDays}));
         }
      }
   ...
   protected void renderMousehoverTooltip(MatrixStack matrices, List<Text> tooltips, int x, int y) {
      if (!tooltips.isEmpty()) {
         int i = 0;
         int j = 0;
         Iterator var7 = tooltips.iterator();
         ...
         for(Iterator var14 = tooltips.iterator(); var14.hasNext(); i += 10) {
   ...
   private void renderLocal(MatrixStack matrices) {
      ...
      RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
      matrices.push();
      matrices.translate((double)(this.width / 2 - 25), 20.0D, 0.0D);
   ...
   public static void loadImages(ResourceManager manager) {
      Collection<Identifier> collection = manager.findResources("textures/gui/images", (filename) -> {
         return filename.endsWith(".png");
      });
      IMAGES = (List)collection.stream().filter((id) -> {
   ...
   static {
      UNINITIALIZED_BUTTON_NARRATION = new TranslatableText("gui.narrate.button", new Object[]{UNINITIALIZED_TEXT});
   ...
   class RealmSelectionListEntry extends RealmsMainScreen.Entry {
      ...
      private void renderMcoServerItem(RealmsServer serverData, MatrixStack matrices, int x, int y, int mouseX, int mouseY) {
         if (serverData.state == RealmsServer.State.UNINITIALIZED) {
            ...
            if (RealmsMainScreen.this.isSelfOwnedServer(serverData) && serverData.expired) {
               ...
               int l = RealmsMainScreen.this.textRenderer.getWidth((StringVisitable)text4) + 17;
               int m = true;
               int n = x + RealmsMainScreen.this.textRenderer.getWidth((StringVisitable)text3) + 8;
               int o = y + 13;
               boolean bl = false;
               ...
               RealmsMainScreen.this.textRenderer.draw(matrices, text3, (float)(x + 2), (float)(q + 1), 15553363);
               DrawableHelper.drawCenteredText(matrices, RealmsMainScreen.this.textRenderer, text4, n + l / 2, q + 1, r);
            } else {
               if (serverData.worldType == RealmsServer.WorldType.MINIGAME) {
                  int s = 13413468;
                  int t = RealmsMainScreen.this.textRenderer.getWidth((StringVisitable)RealmsMainScreen.MINIGAME_TEXT);
      ...
      public Text getNarration() {
         return (Text)(this.mServerData.state == RealmsServer.State.UNINITIALIZED ? RealmsMainScreen.UNINITIALIZED_BUTTON_NARRATION : new TranslatableText("narrator.select", new Object[]{this.mServerData.name}));
      }
   }

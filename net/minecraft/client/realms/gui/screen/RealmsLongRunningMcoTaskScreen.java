...
public class RealmsLongRunningMcoTaskScreen extends RealmsScreen implements Errable {
   ...
   public void init() {
      this.task.init();
      this.cancelButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 - 106, row(12), 212, 20, ScreenTexts.CANCEL, (button) -> {
         this.cancelOrBackButtonClicked();
      }));
   }
   ...
   public void error(Text errorMessage) {
      ...
      this.client.execute(() -> {
         ...
         this.cancelButton = (ButtonWidget)this.addDrawableChild(new ButtonWidget(this.width / 2 - 106, this.height / 4 + 120 + 12, 200, 20, ScreenTexts.BACK, (button) -> {
            this.cancelOrBackButtonClicked();
         }));
      });
   }

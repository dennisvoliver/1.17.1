...
public class ItemRenderer implements SynchronousResourceReloader {
   ...
   public void renderInGui(ItemStack stack, int x, int y) {
      this.innerRenderInGui((LivingEntity)null, stack, x, y, 0);
   }
   ...
   private void innerRenderInGui(@Nullable LivingEntity entity, ItemStack itemStack, int x, int y, int seed, int depth) {
      if (!itemStack.isEmpty()) {
         BakedModel bakedModel = this.getHeldItemModel(itemStack, (World)null, entity, seed);
   ...
   public void renderGuiItemOverlay(TextRenderer renderer, ItemStack stack, int x, int y, @Nullable String countLabel) {
      if (!stack.isEmpty()) {
         ...
         if (stack.getCount() != 1 || countLabel != null) {
            ...
            renderer.draw((String)string, (float)(x + 19 - 2 - renderer.getWidth(string)), (float)(y + 6 + 3), 16777215, true, matrixStack.peek().getModel(), immediate, false, 0, LightmapTextureManager.MAX_LIGHT_COORDINATE);
            immediate.draw();
         }

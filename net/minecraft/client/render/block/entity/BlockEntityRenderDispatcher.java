...
public class BlockEntityRenderDispatcher implements SynchronousResourceReloader {
         ...
         if (blockEntity.hasWorld() && blockEntity.getType().supports(blockEntity.getCachedState())) {
            if (blockEntityRenderer.isInRenderDistance(blockEntity, this.camera.getPos())) {
               runReported(blockEntity, () -> {
                  render(blockEntityRenderer, blockEntity, tickDelta, matrix, vertexConsumers);
               });
            }
         }
      }
   ...
   public void reload(ResourceManager manager) {
      BlockEntityRendererFactory.Context context = new BlockEntityRendererFactory.Context(this, (BlockRenderManager)this.blockRenderManager.get(), this.entityModelLoader, this.textRenderer);

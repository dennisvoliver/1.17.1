...
public class MinecraftClient extends ReentrantThreadExecutor<Runnable> implements SnooperListener, WindowEventHandler {
   ...
   public MinecraftClient(RunArgs args) {
      ...
      this.setOverlay(new SplashOverlay(this, this.resourceManager.reload(Util.getMainWorkerExecutor(), this, (CompletableFuture)COMPLETED_UNIT_FUTURE, (List)list), (throwable) -> {
   ...
   private Profiler startMonitor(boolean active, @Nullable TickDurationMonitor tickDurationMonitor) {
      ...
      if (active) {
         ...
         ++this.trackingTick;
         profiler2 = this.tickTimeTracker.getProfiler();
      } else {
         profiler2 = DummyProfiler.INSTANCE;
      }

      if (this.recorder.isActive()) {
         profiler2 = Profiler.union((Profiler)profiler2, this.recorder.getProfiler());
      }

      return TickDurationMonitor.tickProfiler((Profiler)profiler2, tickDurationMonitor);
   }
   ...
   private Path method_37275(SystemDetails systemDetails, List<Path> list) {
      ...
      try {
         ...
         path2 = RecordDumper.DEBUG_PROFILING_DIRECTORY.resolve(string4);
         ...
         if (var18) {
            ...
            while(var9.hasNext()) {
               ...
               try {
                  ...
                  LOGGER.warn((String)"Failed to delete temporary profiling result {}", (Object)path4, (Object)var19);
               }
            }
   ...
   private void showExperimentalWarning(MinecraftClient.WorldLoadAction worldLoadAction, String levelName, boolean legacyCustomized, Runnable onConfirm) {
      if (worldLoadAction == MinecraftClient.WorldLoadAction.BACKUP) {
         ...
         this.setScreen(new BackupPromptScreen((Screen)null, (shouldBackup, eraseCache) -> {
            if (shouldBackup) {
   ...
   public Text takePanorama(File directory, int width, int height) {
      ...
      try {
         ...
         for(int l = 0; l < 6; ++l) {
            ...
            ScreenshotRecorder.saveScreenshot(directory, "panorama_" + l + ".png", framebuffer, (textx) -> {
            });
         }

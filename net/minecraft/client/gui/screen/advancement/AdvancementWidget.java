...
public class AdvancementWidget extends DrawableHelper {
   ...
   public void drawTooltip(MatrixStack matrices, int originX, int originY, float alpha, int x, int y) {
      ...
      int n;
      if (bl) {
         n = originX + this.x - this.width + 26 + 6;
      } else {
         n = originX + this.x;
      }
      ...
      this.drawTexture(matrices, originX + this.x + 3, originY + this.y, this.display.getFrame().getTextureV(), 128 + advancementObtainedStatus12.getSpriteIndex() * 26, 26, 26);
      if (bl) {
         this.client.textRenderer.drawWithShadow(matrices, (OrderedText)this.title, (float)(n + 5), (float)(originY + this.y + 9), -1);
         if (string != null) {
            this.client.textRenderer.drawWithShadow(matrices, (String)string, (float)(originX + this.x - i), (float)(originY + this.y + 9), -1);
         }
      } else {
         this.client.textRenderer.drawWithShadow(matrices, (OrderedText)this.title, (float)(originX + this.x + 32), (float)(originY + this.y + 9), -1);
         if (string != null) {
            this.client.textRenderer.drawWithShadow(matrices, (String)string, (float)(originX + this.x + this.width - i - 5), (float)(originY + this.y + 9), -1);
         }
      }
      ...
      if (bl2) {
         for(p = 0; p < this.description.size(); ++p) {
            ...
            var22 = (OrderedText)this.description.get(p);
            var10003 = (float)(n + 5);
            var10004 = l + 26 - o + 7;
            Objects.requireNonNull(this.client.textRenderer);
            var21.draw(matrices, var22, var10003, (float)(var10004 + p * 9), -5592406);
         }
      } else {
         for(p = 0; p < this.description.size(); ++p) {
            var21 = this.client.textRenderer;

...
public class TextRenderer {
   ...
   private int draw(OrderedText text, float x, float y, int color, Matrix4f matrix, boolean shadow) {
      VertexConsumerProvider.Immediate immediate = VertexConsumerProvider.immediate(Tessellator.getInstance().getBuffer());
      int i = this.draw((OrderedText)text, x, y, color, shadow, matrix, immediate, false, 0, LightmapTextureManager.MAX_LIGHT_COORDINATE);
      immediate.draw();
      return i;
   }
   ...
   private class Drawer implements CharacterVisitor {
      ...
      public boolean accept(int i, Style style, int j) {
         ...
         if (textColor != null) {
            ...
            m = (float)(k >> 16 & (LightmapTextureManager.MAX_BLOCK_LIGHT_COORDINATE | 15)) / 255.0F * this.brightnessMultiplier;
            n = (float)(k >> 8 & (LightmapTextureManager.MAX_BLOCK_LIGHT_COORDINATE | 15)) / 255.0F * this.brightnessMultiplier;
            o = (float)(k & (LightmapTextureManager.MAX_BLOCK_LIGHT_COORDINATE | 15)) / 255.0F * this.brightnessMultiplier;
         } else {
            m = this.red;
            n = this.green;
            o = this.blue;

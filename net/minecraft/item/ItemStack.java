...
public final class ItemStack {
   ...
   private ItemStack(NbtCompound nbt) {
      this.item = (Item)Registry.ITEM.get(new Identifier(nbt.getString("id")));
   ...
   public List<Text> getTooltip(@Nullable PlayerEntity player, TooltipContext context) {
      ...
      if (this.hasNbt()) {
         ...
         if (isSectionVisible(i, ItemStack.TooltipSection.CAN_DESTROY) && this.nbt.contains("CanDestroy", 9)) {
            nbtList3 = this.nbt.getList("CanDestroy", 8);
            if (!nbtList3.isEmpty()) {
               list.add(LiteralText.EMPTY);
         ...
         if (isSectionVisible(i, ItemStack.TooltipSection.CAN_PLACE) && this.nbt.contains("CanPlaceOn", 9)) {
            nbtList3 = this.nbt.getList("CanPlaceOn", 8);
            if (!nbtList3.isEmpty()) {
               list.add(LiteralText.EMPTY);
   ...
   public boolean hasEnchantments() {
      if (this.nbt != null && this.nbt.contains("Enchantments", 9)) {
         return !this.nbt.getList("Enchantments", 10).isEmpty();
      } else {
         return false;
      }
   }

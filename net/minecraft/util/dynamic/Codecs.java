 ...
 */
public class Codecs {
   public static final Codec<Integer> NONNEGATIVE_INT = rangedInt(0, Integer.MAX_VALUE, (v) -> {
      return "Value must be non-negative: " + v;
   });
   public static final Codec<Integer> POSITIVE_INT = rangedInt(1, Integer.MAX_VALUE, (v) -> {
   ...
   private static <N extends Number & Comparable<N>> Function<N, DataResult<N>> createRangeChecker(N min, N max, Function<N, String> messageFactory) {
      return (value) -> {
         return ((Comparable)value).compareTo(min) >= 0 && ((Comparable)value).compareTo(max) <= 0 ? DataResult.success(value) : DataResult.error((String)messageFactory.apply(value));
      };
   }
